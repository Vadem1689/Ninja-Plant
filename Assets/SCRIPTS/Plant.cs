...
{
    [SerializeField] private SpawnerFruit _spawnerFruit;

    private Fruit _currentFruit;
    [SerializeField] private FruitObject _treeClass;             // Убрать
    [SerializeField] private List<FruitObject> _fruitObject;
    [SerializeField] private SpawnPoint _spawnPoint;           // Убрать
    ...
    private void Update()
    {
        if (_currentFruit == null && _isGrabbing == false)
        {
            ...
            if (_spawnTimeWas > _spawnTime)
            {
                ...
                _currentFruit = _spawnerFruit.SpawnFruit(_treeClass.FruitPrefab, _spawnPointFruit);
